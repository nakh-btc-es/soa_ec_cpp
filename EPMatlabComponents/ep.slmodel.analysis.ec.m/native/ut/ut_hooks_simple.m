function ut_hooks_simple
% Testing the hook calls functionality on low level for normal model workflow.
%
...
[xOnCleanupDoCleanupEnv, xEnv, sResultDir, stTestData] = sltu_prepare_ats_env('model_ref_01', 'UT_EC', sTestRoot);
...
stDefaultInfo = struct( ...
    'sModelPath',           pwd(), ...
    'sModelName',           'main_model', ...
    'sInitFilePath',        '', ...
    'sInitFileName',        '', ...
    'sStubCodeFolder',      fullfile(pwd(), 'main_model_ep_stubs'), ...
    'casReferencedModels',  {{'sub_model_A'; 'sub_model_B'}}, ...
...
for i = 1:numel(casHookNames)
    ...
    switch sHookName
        ...
        case 'ecahook_param_blacklist'
            ...
            SLTU_ASSERT_EQUAL_FLAT_STRUCT(stDefaultInfo, stFoundInfo);
            SLTU_ASSERT_TRUE(isempty(stAutosarMetaInfo), 'Expecting empty AUTOSAR meta info.');
