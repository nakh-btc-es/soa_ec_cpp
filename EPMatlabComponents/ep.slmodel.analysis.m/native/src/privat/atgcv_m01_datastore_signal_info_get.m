function stInfo = atgcv_m01_datastore_signal_info_get(stOpts)
...
%
%   OUTPUT              DESCRIPTION
%       stInfo             (struct)    info about the signal type and structure:
%         .stTypeInfo      (struct)         name of the DataStore
%
%
...
stInfo = i_getSignalInfo(stOpts);
end


%%
function stInfo = i_getSignalInfo(stOpts)
if i_isOptionSet(stOpts, 'sPath')
    stInfo = i_getSignalInfoFromBlock(stOpts.sPath);
else
    stInfo = i_getSignalInfoFromSignalObject(stOpts.sName, stOpts.sModelContext);
end
end


%%
function stInfo = i_getSignalInfoFromBlock(sBlockPath)
...
%
stSigObj = i_getResolvingSignalInfo(sBlockPath);
...
if ~isempty(stSigObj.sInitValue)
    sInitValue = stSigObj.sInitValue;
else
    sInitValue = get_param(sBlockPath, 'InitialValue');
end
hExprResolverFunc = atgcv_m01_expression_resolver_get(sBlockPath);
[xValue, bIsValid] = i_evalExpression(sInitValue, hExprResolverFunc);
if ~bIsValid
    sInitValue = '';
end

% Note: bus init values currently not supported
bIsBus = bIsValid && isstruct(xValue);
if bIsBus
    sInitValue = '';
end
...
if ~isempty(stSigObj.aiDimensions)
    ...
    aiDimensions = i_getDimensions(sBlockPath, hExprResolverFunc, size(xValue));
end
...
%%
function stInfo = i_getResolvingSignalInfo(sBlockPath)
if i_isStateResolvingToSignal(sBlockPath)
    sSigName = get_param(sBlockPath, 'DataStoreName');
    stInfo   = i_getSignalInfoFromSignalObject(sSigName, sBlockPath, i_interpretAsOneDimArray(sBlockPath));
else
...
%%
function stInfo = i_getSignalInfoFromSignalObject(sSignalName, xModelContext, bInterpretAsOneDimArrayBlockSpec)
...
oSignal = feval(hResolverFunc, sSignalName);
if (isempty(oSignal) || ~isa(oSignal, 'Simulink.Signal'))
    stInfo = i_getUnsupportedSignalInfo();
else
    stTypeInfo = ep_sl_type_info_get(oSignal.DataType, hResolverFunc);
    
    hExprResolverFunc = atgcv_m01_expression_resolver_get(xModelContext);
    [sInitialVal, aiDimensions, bDimInherited] = i_getInitValAndDimension(oSignal, hExprResolverFunc);
    ...
    stInfo = i_createSignalInfo( ...
        ...
        sInitialVal, ...
        aiDimensions, ...
        i_getFiniteOrEmpty(oSignal.Min), ...
        i_getFiniteOrEmpty(oSignal.Max));
end
end
...
%%
function [sInitVal, aiDimensions, bDimInherited] = i_getInitValAndDimension(oSignal, hExprResolverFunc)
bDimInherited = false;
...
%%
function stInfo = i_createSignalInfo(stTypeInfo, sInitValue, aiDimensions, dMin, dMax)
stInfo = struct( ...
