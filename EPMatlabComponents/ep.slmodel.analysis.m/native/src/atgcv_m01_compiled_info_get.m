function astBlocks = atgcv_m01_compiled_info_get(varargin)
% Get interface info for the provided blocks in compiled mode.
%
% function astBlocks = atgcv_m01_compiled_info_get(stEnv, casBlocks, sModelDir)
%
%   INPUT               DESCRIPTION
...
casRefMdls = find_mdlrefs(sModel, true);
stRefInfo = struct( ...
    'sModel',  sModel, ...
    'casRefs', {casRefMdls}, ...
    ...
    for j = 1:length(aiSelectedBlocks)
        ...
        try
            [astBlocks(iIdx).astInports, astBlocks(iIdx).astOutports, astBlocks(iIdx).bInnerView] = ...
                atgcv_m01_block_compiled_ports_info_get(stEnv, sBlockPath, astBlocks(iIdx).bInnerView);
            if i_isSfChart(sBlockPath)
                astBlocks(iIdx).astLocals = atgcv_m01_sfchart_locals_info_get(stEnv, sBlockPath);
            end
            astBlocks(iIdx).cadSampleTime = i_getCompiledSampleTime(sBlockPath);
...
%%
function cadSampleTime = i_getCompiledSampleTime(sBlockPath)
cadSampleTime = {};
try
...
if ~isempty(stBlock.astInports)
    bIsComplete = ...
        all([stBlock.astInports(:).bIsInfoComplete]) ...
        && all(arrayfun(@(x) ~isempty(x.iWidth) && (x.iWidth > 0), stBlock.astInports));
    
        % TODO: replace with the following line instead of checking .iWidth
        % && all(arrayfun(@(x) ~isempty(x.aiDim), stBlock.astInports));
end
if (bIsComplete && ~isempty(stBlock.astOutports))
    bIsComplete = ...
        all([stBlock.astOutports(:).bIsInfoComplete]) ...
        && all(arrayfun(@(x) ~isempty(x.iWidth) && (x.iWidth > 0), stBlock.astOutports));
    
        % TODO: replace with the following line instead of checking .iWidth
        % && all(arrayfun(@(x) ~isempty(x.aiDim), stBlock.astOutports));
end
end
