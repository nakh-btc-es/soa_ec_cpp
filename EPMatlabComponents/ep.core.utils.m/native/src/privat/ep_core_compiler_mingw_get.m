function stMEXCompilerSettings = ep_core_compiler_mingw_get(stMEXConfig, sCompilerName)
% Evaluates the compiler settings of a MinGW compiler
%
% function stMEXCompilerSettings = ep_core_compiler_mingw_get(stMEXConfig, sCompilerName)
%
%   INPUT
...
% Compiler struct
stCompiler = struct('sName', sCompilerName, ...
...
% DefaultIncludePaths
% ignore "INCLUDE" environment variable doing mex-option readout
sTmpEnv = getenv('INCLUDE');
if ~isempty(sTmpEnv)
    setenv('INCLUDE', '');
    oCleanup = onCleanup(@()setenv('INCLUDE', sTmpEnv));
end

casIncludePaths = horzcat(casIncludePaths , ep_core_mexopt_read(stMEXConfig, 'INCLUDE', ';'));

if strcmpi(stMEXConfig.Language, 'C++')
    sCppInc = fullfile(stMEXConfig.Location, 'x86_64-w64-mingw32\include\c++');
    if exist(sCppInc, 'dir')
        casIncludePaths{end + 1} = sCppInc;
        casIncludePaths{end + 1} = [sCppInc, '\x86_64-w64-mingw32'];
    end    
end

casIncludePaths{end + 1} = fullfile( ...
    stMEXConfig.Location, ['lib\gcc\x86_64-w64-mingw32\', stMEXConfig.Version, '\include']);
casIncludePaths{end + 1} = fullfile(stMEXConfig.Location, 'x86_64-w64-mingw32\include');
...
for i=1:length(casLinkerFlags)
    ...
    if (~isempty(regexpi(casLinkerFlags{i}, '(.+\.obj"?)$')))
        ...
        if ~isempty(casRemaining)
            ...
            for j=1:length(casRemaining)
                ...
                if isempty(regexpi(sLinkerFlag, '(.+\.obj"?)$')) || ...
...
function [bOmit] = i_is_linker_flag_excluded(sLinkerFlag)
    ...
    if ismember(sLinkerFlag, {'llibmx', 'llibmex', 'llibmat', 'lm', 'llibmwlapack', 'llibmwblas', ...
