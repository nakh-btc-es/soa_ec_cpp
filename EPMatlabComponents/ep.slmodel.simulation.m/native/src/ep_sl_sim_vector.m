function [stExtractInfo, stExtractOpenInfo, bSimSuccessful] = ep_sl_sim_vector(varargin)
...
%
%         Kind                    (TL|SL) kind of model, (TL for TargetLink Model, SL for Simulink Model, default:SL)
%                                 ATTENTION: Since we split up these domains, only kind "SL" is allowed here.
%                                            See also ep_tl_sim_vector.
...
%         ActivateMil  (boolean)  TRUE if MIL mode should be activated permanently (default: true) (TargetLink
%                                 Option, Kind = TL) the model file is already loaded.
%
...
ep_sim_argcheck('Kind', stArgs, {'class', 'char'}, {'keyvalue_i', {'SL', 'TL'}});
ep_sim_argcheck('OriginalSimulationMode', stArgs, {'class', 'char'}, {'keyvalue_i', {'SL MIL', 'SL SIL', 'SL MIL (Toplevel)'}});
