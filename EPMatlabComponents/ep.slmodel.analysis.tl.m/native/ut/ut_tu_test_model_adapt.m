function ut_tu_test_model_adapt(varargin)
...
bUseCaching = ut_use_caching();

sBackupDir = ut_cache_dir_get();
...
aiSha256 = uint8(jHasher.ComputeHash(uint8(sString)));
sString = reshape(dec2hex(aiSha256), 1, []);
end
...
cd('..\..'); 
ut_copyfile(fullfile(sModelBackupDir, '*'), sModelTestDir, 'f');
