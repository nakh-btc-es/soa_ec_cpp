function ut_ep_matrix_support_03
% Test the matrix support feature
%
%  ut_ep_matrix_support_03
%
%  INPUT             DESCRIPTION
%
%  OUTPUT            DESCRIPTION
...
sPwd      = pwd();
sTestRoot = fullfile(sPwd, 'ut_ep_matrix_support_03');
sDataDir  = fullfile(ut_local_testdata_dir_get(), 'matrix_sig_models', 'matrix_sig3', 'tl40');
...
i_check_interface_object(hTlRoot, 'inport', 'In1', [], 2, 3, 'double', 'fixedPoint', 1);
...
ahInports = mxx_xmltree('get_nodes', hSlRoot, ...
    '//inport[@path="top_A/In1"]');
MU_ASSERT_TRUE(~isempty(ahInports), ...
    '"top_A/In1" is missing');
...
for i = 1:length(ahIoMappingIn1);
    ...
    MU_ASSERT_TRUE(~isempty(mxx_xmltree('get_nodes', hIoMappingIn1, './Path[@refId="id1" and @path="Sa1_In1"]')));
