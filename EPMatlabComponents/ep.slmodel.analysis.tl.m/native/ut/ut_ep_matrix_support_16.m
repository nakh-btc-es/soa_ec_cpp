function ut_ep_matrix_support_16
% Test the matrix support feature
%
%  ut_ep_matrix_support_16
%
%  INPUT             DESCRIPTION
%
%  OUTPUT            DESCRIPTION
...
sPwd      = pwd();
sTestRoot = fullfile(sPwd, 'ut_ep_matrix_support_16');
sDataDir  = fullfile(ut_local_testdata_dir_get(), 'matrix_sig_models', 'matrix_sig16', 'tl40');
...
i_check_interface_object(hTlRoot, 'inport', 'sub_B2/In1', [], 1, 4, 'double', 'fixedPoint');
i_check_interface_object(hTlRoot, 'outport', 'sub_B2/Out1', [], 1, 4, 'double', 'fixedPoint');
i_check_interface_object(hTlRoot, 'display', 'sub_B2/Gain', [], 1, 4, 'double', 'fixedPoint');

i_check_interface_object(hTlRoot, 'inport', 'sub_B3/In1', [], 2, 2, 'double', 'fixedPoint');
i_check_interface_object(hTlRoot, 'outport', 'sub_B3/Out1', [], 2, 2, 'double', 'fixedPoint');
i_check_interface_object(hTlRoot, 'display', 'sub_B3/Gain', [], 2, 2, 'double', 'fixedPoint');

i_check_interface_object(hTlRoot, 'inport', 'sub_B4/In1', [], 1, 2, 'double', 'fixedPoint');
i_check_interface_object(hTlRoot, 'outport', 'sub_B4/Out1', [], 1, 2, 'double', 'fixedPoint');
i_check_interface_object(hTlRoot, 'display', 'sub_B4/Gain', [], 1, 2, 'double', 'fixedPoint');

i_check_interface_object(hTlRoot, 'inport', 'sub_B5/In1', [], 2, 1, 'double', 'fixedPoint');
i_check_interface_object(hTlRoot, 'outport', 'sub_B5/Out1', [], 2, 1, 'double', 'fixedPoint');
i_check_interface_object(hTlRoot, 'display', 'sub_B5/Gain', [], 2, 1, 'double', 'fixedPoint');
...
ahIoMapping = mxx_xmltree('get_nodes', hMapRoot, ...
    ...
    '/Path[@path="In1"]/..']);
MU_ASSERT_TRUE(length(ahIoMapping) == 4);
aiCIndex = [2,1,5,3];
for i = 1:length(ahIoMapping);
    ...
    MU_ASSERT_TRUE(~isempty(mxx_xmltree('get_nodes', hSignalMapping, ...
        ['./Path[@refId="id1" and @path="[',num2str(aiCIndex(i)),']"]'])));
end
...
% test 'In1' on  'top_A/Subsystem/top_A/sub_B5'
ahIoMapping = mxx_xmltree('get_nodes', hMapRoot, ...
    ['//ScopeMapping/Path[@path="top_A/Subsystem/top_A/sub_B5"]/../InterfaceObjectMapping[@kind="Input"]', ...
    '/Path[@path="In1"]/..']);
MU_ASSERT_TRUE(length(ahIoMapping) == 2);
aiCIndex = [1,2;0,2];
for i = 1:length(ahIoMapping);
...
for i = 1:length(ahIoMapping);
    ...
    hSignalMapping = mxx_xmltree('get_nodes', hIoMappingIn1, './SignalMapping');
    MU_ASSERT_TRUE(~isempty(mxx_xmltree('get_nodes', hSignalMapping, ...
        ['./Path[@refId="id0" and @path=""]'])));
    MU_ASSERT_TRUE(~isempty(mxx_xmltree('get_nodes', hSignalMapping, ...
        ['./Path[@refId="id1" and @path="[',num2str(2),']"]'])));
end
end