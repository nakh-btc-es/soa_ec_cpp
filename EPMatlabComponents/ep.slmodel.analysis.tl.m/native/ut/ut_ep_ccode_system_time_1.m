...
sPwd      = pwd();
sTestRoot = fullfile(sPwd, 'ut_ep_ccode_system_time_1');
sDataDir  = fullfile(ut_local_testdata_dir_get(), 'system_time_float');
...
while ischar(sLine)
    ...
    elseif strncmp('void ',sLine, 5)
        ...
        sTime = sLine(length('void system_time_') + 1 : iIdx);
        sTime = strrep(sTime, '_', '.');
        iIncrement = str2double(sTime);
