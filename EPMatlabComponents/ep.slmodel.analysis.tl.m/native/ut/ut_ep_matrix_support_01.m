function ut_ep_matrix_support_01
% Test the matrix support feature
%
%  ut_ep_matrix_support_01
%
%  INPUT             DESCRIPTION
%
%  OUTPUT            DESCRIPTION
...
sPwd      = pwd();
sTestRoot = fullfile(sPwd, 'ut_ep_matrix_support_01');
sDataDir  = fullfile(ut_local_testdata_dir_get(), 'matrix_sig_models', 'matrix_sig1', 'tl40');
...
for i = 1:length(ahIoMappingIn1)
    ...
    MU_ASSERT_TRUE(~isempty(mxx_xmltree('get_nodes', hSignalMapping, ['./Path[@refId="id0" and @path="(1)(',num2str(i),')"]'])));
    MU_ASSERT_TRUE(~isempty(mxx_xmltree('get_nodes', hSignalMapping, ['./Path[@refId="id2" and @path="(1)(',num2str(i),')"]'])));
...
i_check_interface_object(hTlRoot, 'inport', 'In1', [], 1, 3, 'double', 'fixedPoint', 1);
