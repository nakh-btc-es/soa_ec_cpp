function ut_bug_25046
% UPDATED LEGACY UT
% Old name: ut_mt01_bug_25046
%
%   REMARK
%    * BTS/25046
%      Error situation: Multi-port switch is allowed to have different
%      input width: if the control_signal (index) is multi-dimensional and
%      the inputs are scalar, the output is expanded to be also
%      multi-dimensional (vector) with the same width as the control signal
%      However, the Signal back-propagation algo is not prepared to handle
%      multi-port blocks where the dimension of the output is not the same
%      as the dimension of the inputs.
%      --> Fix: Adapt backprop-algo.
%
% checking fix for BTS/25046
