function ut_ep_matrix_support_06
% Test the matrix support feature
%
%  ut_ep_matrix_support_05
%
%  INPUT             DESCRIPTION
%
%  OUTPUT            DESCRIPTION
...
sPwd      = pwd();
sTestRoot = fullfile(sPwd, 'ut_ep_matrix_support_06');
sDataDir  = fullfile(ut_local_testdata_dir_get(), 'matrix_sig_models', 'matrix_sig6', 'tl40');
...
i_check_interface_object(hTlRoot, 'outport', 'Out2', [], 2, 3, 'double', 'fixedPoint', 1);
...
ahInports = mxx_xmltree('get_nodes', hSlRoot, ...
    '//outport[@path="top_A/Out2"]');
MU_ASSERT_TRUE(~isempty(ahInports), ...
    '"top_A/Out2" is missing');
...
i_check_matrix_sl(hInport, '2', '1');
i_check_matrix_sl(hInport, '2', '2');
i_check_matrix_sl(hInport, '2', '3');
end
...
ahIoMappingIn1 = mxx_xmltree('get_nodes', hMapRoot, ...
    '//ScopeMapping/Path[@path="top_A/Subsystem/top_A"]/../InterfaceObjectMapping[@kind="Output"]');
...
for i = 1:length(ahIoMappingIn1);
    ...
    MU_ASSERT_TRUE(~isempty(mxx_xmltree('get_nodes', hIoMappingIn1, './Path[@refId="id1" and @path="Sa1_Out2"]')));
...
%% helper
function i_check_matrix_sl(hInport, sX, sY)
ahSignal = mxx_xmltree('get_nodes', hInport, ...
    ['./nonUniformArray/signal[@index="', sX,'"]']);
MU_ASSERT_TRUE(~isempty(ahSignal), ['1st index of (', sX,',', sY,') Signal node is missing']);

ahSignal = mxx_xmltree('get_nodes', hInport, ...
    ['./nonUniformArray/signal[@index="', sX,'"]/nonUniformArray/signal[@index="', sY,'"]']);
MU_ASSERT_TRUE(~isempty(ahSignal), ['2nd index of (', sX,',', sY,') Signal node is missing']);

ahSignalDataType = mxx_xmltree('get_nodes', hInport, ...
    ['./nonUniformArray/signal[@index="', sX,'"]/nonUniformArray/signal[@index="', sY,'"]/double']);
MU_ASSERT_TRUE(~isempty(ahSignalDataType), 'Signal data type node is missing');
end
