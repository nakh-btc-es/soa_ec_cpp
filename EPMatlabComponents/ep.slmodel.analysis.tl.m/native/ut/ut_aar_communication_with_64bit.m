function ut_aar_communication_with_64bit
%
% Checking that for TL Adaptive Autosar models the 64bit inputs/outputs are not rejected. 
% The demo model is for TL versions >= 5.2 
%
...
if verLessThan('tl', '5.2')
    MU_MESSAGE('TEST SKIPPED: Testdata with "Adaptive AUTOSAR" only for TL5.2 and higher.');
    return;
end
...
%% assert
if verLessThan('tl', '22.1')
    sSubFolder = 'tl52';
else
    sSubFolder = 'tl221';
end

sExpectedTlArch = fullfile(sTestDataDir, sSubFolder, 'TlArch.xml');
...
sExpectedMessageFile = fullfile(sTestDataDir, sSubFolder, 'error.xml');
