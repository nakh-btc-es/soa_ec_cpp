function stInfo = atgcv_m01_variable_class_get(stEnv, hVar)
...
%%
hVar = i_normalizeVariable(stEnv, hVar);

hClass = [];
if ~isempty(hVar)
    hClass = atgcv_mxx_dsdd(stEnv, 'GetClass', hVar);
end
hClass = i_normalizeClass(hClass);
stInfo = i_getInfoClass(stEnv, hClass);
...
function stInfo = i_getInfoClass(stEnv, hClass)
stInfo = struct( ...
    'hClass',       [], ...
    'sScope',       '', ...
    'sStorage',     '', ...
...
stInfo.sScope   = atgcv_mxx_dsdd(stEnv,  'GetScope',   hClass);
stInfo.sStorage = atgcv_mxx_dsdd(stEnv,  'GetStorage', hClass);
stInfo.bIsMacro = logical(atgcv_mxx_dsdd(stEnv, 'GetMacro',  hClass));
stInfo.bIsConst = logical(atgcv_mxx_dsdd(stEnv, 'GetConst',  hClass));
end
...
%%
function hVar = i_normalizeVariable(stEnv, xVar)
...
% variable from it
sKind = atgcv_mxx_dsdd(stEnv, 'GetAttribute', hVar, 'objectKind');
